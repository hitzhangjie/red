...
//
// this package is based on https://github.com/hokaccha/go-prettyjson,
// and modified to avoid number precision loss.
package prettyjson
...
func (f *Formatter) Marshal(v interface{}) ([]byte, error) {
	...
	log.Println("data before format:", string(data))
...
// 注意：
// - 对于数值类型，如果dec.UseNumber()未调用，则解码为float64，反之解码为json.Number，
// - 对于个别其他不支持的数据类型，打印其类型信息+<skip>提示
func (f *Formatter) pretty(v interface{}, depth int) string {
	...
	case json.Number:
		return f.sprintColor(f.NumberColor, fmt.Sprintf("%v", val))
	case bool:
